...
function godspeed_gform_validation_message($message, $form)
{
...
add_filter('gform_validation_message', 'godspeed_gform_validation_message', 10, 2);

*/

// This filter can be used to dynamically add/remove CSS classes to a field
function godspeed_gform_field_css_class($classes, $field, $form)
{
...
add_action('gform_field_css_class', 'godspeed_gform_field_css_class', 10, 3);
...
function godspeed_gform_submit_button($button, $form)
{
  return '
...
add_filter('gform_submit_button', 'godspeed_gform_submit_button', 10, 2);

function godspeed_gform_next_button($button, $form)
{
    return '<div class="form-actions form-actions-next">'.str_replace('gform_next_button button','btn btn-primary',$button).'</div>';
}
add_filter('gform_next_button', 'godspeed_gform_next_button', 10, 2);

function godspeed_gform_previous_button($button, $form)
{
    return '<div class="form-actions form-actions-previous">'.str_replace('gform_previous_button button','btn btn-link',$button).'</div>';
}
add_filter('gform_previous_button', 'godspeed_gform_previous_button', 10, 2);
...
function godspeed_gform_form_tag($form_tag, $form)
{
  $class = "form ";
...
add_filter("gform_form_tag", "godspeed_gform_form_tag", 10, 2);
...
function godspeed_gform_field_content($content, $field, $value, $lead_id, $form_id)
{
  global $form_css;
  if(!$form_css){
    $form_css='';
...
add_filter("gform_field_content", "godspeed_gform_field_content", 10, 5);
...
function godspeed_gform_field_choices($choices, $field)
{
...
add_filter("gform_field_choices", "godspeed_gform_field_choices", 10, 5);

/*
// This filter can be used to dynamically change the confirmation message or redirect URL for a form
add_filter("gform_confirmation", "godspeed_gform_confirmation", 10, 4);
function godspeed_gform_confirmation($confirmation, $form, $lead, $ajax){
  if($form['confirmation']['type']=='message')
    return godspeed_alert(array("type"=>"success"),$confirmation);
  else
    return $confirmation;
}


//add_filter("gform_cdata_open", "godspeed_gform_cdata_open", 10, 4);
function godspeed_gform_cdata_open($content)
{
  return 'document.onload = function() {';
}

//add_filter("gform_cdata_close", "godspeed_gform_cdata_close", 10, 4);
function godspeed_gform_cdata_close($content)
{
  return '}';
}
*/
add_filter("gform_get_form_filter", "godspeed_gform_get_form_filter", 10, 4);
function godspeed_gform_get_form_filter($content)
{
...
add_filter('body_class', 'godspeed_body_class_useractivation');
function godspeed_body_class_useractivation($classes = '') {
